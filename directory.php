<!DOCTYPE html>
<html lang="en">

<?php include("includes.php");
      printHead();?>

<body>

    <!-- Navigation -->
    <?php include("header.php");
          printNav(4);?>


    <div class="intro about-intro">
        <div class="intro-body">
            <div class="container">
                <div class="row">
                    <div class="col-md-1"></div>
                    <div class="col-md-10 text-background-intro">
                        <h1 class="brand-heading" style="margin-bottom:0">Brother Directory</h1>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="container">
        <table class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>First Name</th>
                    <th>Last Name</th>
                    <th>
                        email
                    </th>
                    <th>Office
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>Alec</td>
                    <td>Pierce</td>
                    <td> apierce2@wisc.edu</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td>Austin</td>
                    <td>Mease</td>
                    <td>amease@wisc.edu</td>
                    <td>Polygon</td>
                </tr>
                <tr>
                    <td>Blake</td>
                    <td>Pluemer</td>
                    <td>bpluemer@wisc.edu</td>
                    <td>IFC</td>
                </tr>
                <tr>
                    <td>Daniel</td>
                    <td>Hayes</td>
                    <td>dahayes@wisc.edu</td>
                    <td>Treasurer</td>
                </tr>
                <tr>
                    <td>Evan</td>
                    <td>Schertz</td>
                    <td>eschertz@wisc.edu</td>
                    <td>VP, Kitchen, Secretary</td>
                </tr>
                <tr>
                    <td>Evan</td>
                    <td>Kivolowitz</td>
                    <td>ekivolowitz@wisc.edu</td>
                    <td>Fundraising, Social</td>
                </tr>
                <tr>
                    <td>Grant </td>
                    <td>Herrman</td>
                    <td>gherrman@wisc.edu</td>
                    <td>Expo</td>
                </tr>
                <tr>
                    <td>Jason</td>
                    <td>Salzsieder</td>
                    <td>salzsieder@wisc.edu</td>
                    <td>House Manager</td>
                </tr>
                <tr>
                    <td>John</td>
                    <td>Klopatek</td>
                    <td>jklopatek@wisc.edu</td>
                    <td>Alumni Relations</td>
                </tr>
                <tr>
                    <td>Johnathan</td>
                    <td>Brandenburg</td>
                    <td>jjbrandenbur@wisc.edu</td>
                    <td>PR/ Communications </td>
                </tr>
                <tr>
                    <td>Kevin</td>
                    <td>Clark</td>
                    <td>kwclark@wisc.edu</td>
                    <td>President</td>
                </tr>
                <tr>
                    <td>Kyle</td>
                    <td>Sirbasku</td>
                    <td> sirbasku@wisc.edu</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td>Matt</td>
                    <td>Glattfelder</td>
                    <td>glattfelder@wisc.edu</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td>Mike</td>
                    <td>Pyrtko</td>
                    <td>pyrtko@wisc.edu</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td>Mitchel</td>
                    <td>Wanta </td>
                    <td>mswanta@wisc.edu</td>
                    <td>Member Relations/RM, Editor</td>
                </tr>
                <tr>
                    <td>Paul</td>
                    <td>Smith</td>
                    <td>psmith24@wisc.edu</td>
                    <td>IFC, Info Systems</td>
                </tr>
                <tr>
                    <td>Stephen</td>
                    <td>Mcmanus</td>
                    <td>smcmanus2@wisc.edu</td>
                    <td>Recruitment, Academics, Philanthropy, Special Events</td>
                </tr>
                <tr>
                    <td>Steven</td>
                    <td>Tang</td>
                    <td>rtang37@wisc.edu</td>
                    <td>Assistant Recruitment, Office</td>
                </tr>
                <tr>
                    <td>Steven</td>
                    <td>Zastrow</td>
                    <td>szastrow2@wisc.edu</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td>Will </td>
                    <td>Leng</td>
                    <td>sleng3@wisc.edu</td>
                    <td>Comptroller</td>
                </tr>
                <tr>
                    <td>Xiao</td>
                    <td>He</td>
                    <td>xhe66@wisc.edu</td>
                    <td>Assistant Recruitment, Occupancy, Photo</td>
                </tr>
            </tbody>
        </table>
        </table>
    </div>


    <!-- Footer -->
    <?php printFooter(); ?>

</body>

<!-- script -->
<?php printScripts(); ?>

</html>
